...
class Query {
    ...
    async execute(): Promise<this> {
        try {
            ...
            // Copy all properties from the result
            this.rowCount = res.rowCount || 0;
            this.rows = res.rows || [];
            this.command = res.command || "";
            this.fields = res.fields || [];
            this.oid = res.oid || null;
            this.#error = null;
        } catch (error) {
            this.#error =
                error instanceof Error ? error : new Error(String(error));
            // You might want to throw or handle the error differently
            console.error("Query execution error:", this.#error.message);
        }

        // Always return this for consistent chaining
        return this;
    }
    ...
    // Custom toString to show the query
    toString() {
        return this.#query;
    }
